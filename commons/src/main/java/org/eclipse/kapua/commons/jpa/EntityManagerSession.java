...
public class EntityManagerSession {
    ...
    private <T> void appendKapuaEvent(EntityManager manager) throws KapuaException {
        appendKapuaEvent(null, manager);
    }

    private <T> void appendKapuaEvent(Object instance, EntityManager manager) throws KapuaException {
        ...
        if (!(instance instanceof EventStoreRecord)) {
            ...
            if (serviceEventBus != null) {
                ...
                if (isNewEvent(serviceEventBus)) {
                    EventStoreRecord persistedKapuaEvent = EventStoreDAO.create(manager, ServiceEventUtil.fromServiceEventBus(serviceEventBus));
                    // update event id on Event (persistedKapuaEvent.getId() cannot be null since is generated by the database)
